...
class PGGAN(object):
    def __init__(self,
        ...
        self.stable_and_fadein_step = [1,1,1,1,1,1,1]  # 1代表训练1轮
        ...
        self._init_network()
        self.avg_layer = torch.nn.AvgPool3d((2,2,2),stride=(2,2,2))
...
#                     net_alpha = 1.0 - (step + 1) / fadein_steps
    ...
    def _train(self, net_level, net_status, net_alpha, cur_step):
        ...
        current_level_res = 2 ** (net_level + 1)  # 当前层的resolution
        ...
        for batch_idx, (spec, IF, pitch_label, mel_spec, mel_IF) in enumerate(self.dataloader.train_loader): 
            ...
            while stack_real_image.size()[2] != current_level_res:  # 压缩真实图像
            ...
            if net_status =='stable':
                ...
                if little_batch_size==BATCH_SIZE:
                    net_alpha = 1.0 - (cur_step * TOTAL_DATA_SIZE + batch_idx * little_batch_size) / (self.fadein_steps * TOTAL_DATA_SIZE)  # 当前迭代次数/总迭代次数
                else:
            ...
            fake_one_hot_pitch_condition_vector = torch.zeros(little_batch_size, 128).scatter_(1, fake_pitch_label, 1).unsqueeze(2).unsqueeze(3).cuda()  # 生成pitch label向量
            fake_pitch_label = fake_pitch_label.cuda().squeeze()


            """ generate random vector """
            fake_seed = torch.randn(little_batch_size, self.latent_size_, 1, 1).cuda()  # 随机噪音
            fake_seed_and_pitch_condition = torch.cat((fake_seed, fake_one_hot_pitch_condition_vector), dim=1)  # 噪音和label拼接

            fake_generated_sample = self.g_net(fake_seed_and_pitch_condition)  # 这里的输入改成脑电
            ...
            real_pitch_loss = self.NLL_loss(pitch_real, pitch_label)  # NLLloss+softmax层=交叉熵
