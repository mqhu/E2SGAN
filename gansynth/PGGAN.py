...
'''添加一种新的level叫做last，用来处理生成器信道数量。同时HelpFunc也要跟着一起修改一下。'''
...
class HelpFunc(object):
    ...
    def process_transition(a, b):
        ...
        :return torch.Tensor :
        """
        a_batch, a_channel, a_height, a_width = a.shape  # channel是特征层数，electrode是电极数
    ...
    *'To initialize all bias parameters to zero and all weights
    ...
    def forward(self, x):
        ...
        if self.bias_ is not None:
            ...
            x += self.bias.view(1, self.bias.size()[0], 1, 1)  # 一个卷积核配一个bias
        return x
...
class Generator(nn.Module):
    def __init__(self,
                 resolution,  # Output resolution. Overridden based on dataset.  似乎没什么用
    ...
    def forward(self, x):
        ...
        if self.net_status_ == "stable":
            ...
            #print("net_level:", self.net_level_)
            for cursor in range(self.net_level_+1):
                x = self.lod_layers_[cursor](x)  # 把一层的输出当做下一层的输入
            x = self.rgb_layers_[cur_output_level](x)

        elif self.net_status_ == "fadein":  # 只有在加入新层时进行fade in
            ...
            pre_x = HelpFunc.process_transition(output_cache[0], output_cache[1])
            pre_x = self.rgb_layers_[cur_output_level](pre_x)
            cur_x = self.rgb_layers_[cur_output_level](output_cache[1])
            x = pre_x * pre_weight + cur_x * cur_weight  # 分辨率过渡
            '''
    ...
    def _construct_by_level(self, cursor):
        ...
        # if out_level == self.net_level_max_:
        #     self._create_block(out_channels, self.final_channel_, "ToRGB_final")
        # else:
    ...
    def _create_block(self, in_channels, out_channels, block_type):
        ...
        if block_type in ["First", "UpSample"]:
            ...
            # if "final" in block_type:
            #     block_cache.append(networks.TransposeHelper(2, 1))
            #     block_cache.append(nn.Conv3d(16, 17, kernel_size=1, stride=1, padding=0, bias=False))  # 这里是为了还原信道数
            #     block_cache.append(networks.TransposeHelper(1, 2))
            #     block_cache.append(EqualizedLearningRateLayer(block_cache[-2]))
            #     block_cache.append(nn.LeakyReLU(negative_slope=0.2))
            #     block_cache.append(PixelWiseNormLayer())
            # block_cache.append(nn.LeakyReLU(negative_slope=0.2))
            # block_cache.append(PixelWiseNormLayer())
...
class Discriminator(nn.Module):
    ...
    @property  # 把net_config当做一个属性来用
    def net_config(self):
        return self.net_level_, self.net_status_, self.net_alpha_

    @net_config.setter  # 修改net_config这个属性
    def net_config(self, config_list):
        self.net_level_, self.net_status_, self.net_alpha_ = config_list
    ...
    def _construct_by_level(self, cursor):
        ...
        if self.channel_list is not None:  # channel_list指定每一层的卷积核数量
            in_channels=self.channel_list[in_level]
            out_channels=self.channel_list[out_level]
            print("Cursor",cursor,in_channels,out_channels)
        else:  # 不指定的话就按照层数递推
            in_channels, out_channels = map(self._get_channel_by_stage, (in_level, out_level))  # 得到这两层的图像channel数
        ...
        if block_type == "Minibatch":  # minibatch层是最后一层
            #self.pitch_classifier= nn.Linear(self.channel_list[0]*2*16, 128)
            self.discriminator_classifier= nn.Linear(self.channel_list[0]*16*16, 1)  # 维度改为一个n_chan*width*height
    ...
    def _get_channel_by_stage(self, level):  # 得到当前阶段图像的channel数
        return min(int(self.fmap_base_ / (2.0 ** (level * self.fmap_decay_))), self.fmap_max_)