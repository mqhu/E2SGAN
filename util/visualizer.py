...
class Statistics:
    ...
    def save_plotted_losses(self, epoch, epoch_counter, counter_ratio, losses):
        ...
        if not hasattr(self, 'plot_data'):
            self.plot_data = {'X': [], 'Y': [], 'legend': list(losses.keys())} # losses是一个字典
    ...
    def save_score_plots(self, freq, score_dict):
        '''输入的score_dict是个字典，包含了score的名字和累计值，累计值需要转置后使用'''
...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        im = util.tensor2im(im_data) #这里将tensor转化成了numpy三通道格式
        image_name = '%s_%s.png' % (name, label) #保存为png图片
