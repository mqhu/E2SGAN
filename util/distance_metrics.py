...
band_width = 12
#eeg_bands = ([0, 4], [4, 7], [8, 15], [16, 31], [32, 80])
# bands = ([0, 4], [5, 7], [8, 15], [16, 28], [0, 28])
...
def get_bands():
    ...
    for i in range(0, conf.seeg_ceiling_freq, band_width):  # 获取每个频段区间的端点值
...
def get_z_score_params(real_path, save_path=None):
    ...
    v_sum = [0 for _ in range(n_band)]  # 最后一个元素存放所有维度的和、均值和方差
    stdv = [0 for _ in range(n_band)]
    num = [len(dataset) for _ in range(n_band)]   # 保存每个频段区间的样本点数量
    flag = True
    v = []
    z_score_params = np.load("/home/hmq/Infos/z_score_params/stft_Fz_224x224_ae_e2s_28Hz_z_score.npy", allow_pickle=True).item()
    ...
    for i, f_name in enumerate(dataset):
        ...
        real = np.load(os.path.join(real_path, f_n + '.npy'))[0]  # 转化成1维数据
...
def shrinkage_estimator(weights, avg_dist, q=0.1):
    ...
    :param weights: np.ndarray, (n_band, ，波段的权重，即波段内能量总和，归一化为0~1且总和为1
    :param avg_dist: np.ndarray，(n_band, 2), mag各波段求出的真实与生成值的差的均值，有L1和L2两种结果
    :param q: 超参，默认为0.1
    :return: np.ndarray, (n_band, 2), 收缩过后的平均距离
    '''
...
def calculate_distance(real_path, fake_path, normalizer, z_score_path=None, method='GAN', is_IF=False, aggregate=True, save_path=None, aux_normalizer=None, baseline=None):
    ...
    :param aggregate: 是否聚合所有病人的结果
    :param real_path: 真实信号路径, 不包括病人名
    :param fake_path: 生成信号路径
    :param normalizer: normalizer
    :param z_score_path: 暂时不需要
    :param method: str | 'EEG','GAN', 'EUD_temporal', 'EUD_freq', 'spline', 'asae'
    :param save_path: 保存result的目录
    :return: result
    '''
    ...
    # z_score_params = np.load(z_score_path, allow_pickle=True).item()
    # v_sum = z_score_params['sum']
    # mean = z_score_params['mean']
    # stdv = z_score_params['stdv']
    # n_band = len(bands) - 1
    n_band = len(bands)
    # weights = np.asarray([s / sum(v_sum) for s in v_sum])
    patient_result = {}
    ...
    for i, f_name in enumerate(dataset):
        ...
        # elif method in ['phase']:
        #     real_mag = np.load(os.path.join(fake_path, f_n + '_real_B.npy'))
        #     fake = np.concatenate((real_mag, fake), axis=0)
        #     fake = IF_to_eeg(fake, normalizer, iseeg=False, is_IF=is_IF)[0]
        # elif method in ['pghi']:
        #     postprocessor = AudioPreprocessor(sample_rate=conf.seeg_sf,
        #                                   audio_length=conf.audio_length,
        #                                   transform='pghi', hop_size=conf.seeg_hop, stft_channels=conf.seeg_n_fft).get_post_processor()
        #     fake = pghi_invert(fake, normalizer, postprocessor, iseeg=False)[0]

        real = np.load(os.path.join(real_path, patient, f_n + '.npy'))[0]  # 转化成1维数据yasa使用
        if len(real) > fake.shape[0]:
            real = real[: fake.shape[0]]
        r_temp, r_mag, _, _ = get_time_freq_by_band(real, 0, conf.seeg_ceiling_freq, iseeg=False)  # 将真实的信号最高频率与生成的对齐

        fake = fake.astype(np.float64)  # transfer original signal to float64 for the fake signal is generated float32
        # print('fake temp shape:', fake.shape)
        # r_downsampled = mne.filter.resample(r_temp, down=4, npad='auto')
        # f_downsampled = mne.filter.resample(fake, down=4, npad='auto')
        # r_temp = (r_temp - mean[-1]) / stdv[-1]
        # f_temp = (fake - mean[-1]) / stdv[-1]

        # if method in ['GAN', 'EUD_freq', 'phase']:
            # dtw_dist, _, _, _ = accelerated_dtw(r_temp, fake, dist='euclidean')
        ...
        if method in ['EEG', 'EUD_temporal', 'spline', 'asae', 'EEGGAN']:
            ...
            # rmse_dist = np.linalg.norm(r_temp - fake, ord=2) / len(r_temp) ** 0.5
        # if method in ['EEG']:
        #     f_min, f_max = np.min(fake), np.max(fake)
        #     r_min, r_max = np.min(r_temp), np.max(r_temp)
        #     k = (f_max - f_min) / (r_max - r_min)
        #     b = (r_max * f_min - f_max * r_min) / (r_max - r_min)
        #     r_temp = r_temp * k + b
        ...
        for j, tup in enumerate(bands):
            ...
            # 计算相位/IF的海明距离
            if j == len(bands) - 1:
                # patient_result[patient]['phase_dist'].append(hamming_distance(r_p, f_p))
                # patient_result[patient]['phase_dist'].append(ham_dist(phash(r_p), phash(f_p)))
                # 归一化psd，计算hd和bd
                ...
                # print('real max/min:{}/{}'.format(np.max(r_psd_prob), np.min(r_psd_prob)))
                # print('fake max/min:{}/{}'.format(np.max(f_psd_prob), np.min(f_psd_prob)))
            ...
            # r_mag = (r_mag - mean[j]) / stdv[j]
            # f_mag = (f_mag - mean[j]) / stdv[j]
            # if j < 4:
            #     r_downsampled = mne.filter.resample(real, down=2, npad='auto')
            #     f_downsampled = mne.filter.resample(fake, down=2, npad='auto')
            #     dtw_dist, _, _, _ = accelerated_dtw(r_downsampled, f_downsampled, dist='euclidean')
            #     temporal_dist.append(dtw_dist)
            # else:
            #dtw_dist, _, _, _ = accelerated_dtw(r_temp, f_temp, dist='euclidean')
            #temporal_dist[j][2] += dtw_dist
            #mag_dist[i][0] += wasserstein_distance(r_mag, f_mag)
            # mag_dist[j][0].append(np.linalg.norm(r_mag - f_mag, ord=1))
            # mag_dist[j][1].append(np.linalg.norm(r_mag - f_mag))
            #phase_dist[i][0] += wasserstein_distance(r_p, f_p)
            # phase_dist[j].append(np.sum((r_p - f_p) != 0))  # 统计两个数组对应元素不相等的个数
            #phase_dist[j][1] += np.linalg.norm(r_p - f_p)
        # shrunk_mag_dist = shrinkage_estimator(weights, mag_mean)
        print('Temporal:')
        print(dtw_dist)
        # print('Phase:')
        # print(np.asarray(phase_dist) / (i + 1))
        # if style == 'EMD':
        #     dist += wasserstein_distance(sig_A[i], sig_B[i])
        # elif style == 'L1':
        #     dist += np.linalg.norm(sig_A[i] - sig_B[i], ord=1)
        # elif style == 'L2':
        #     dist += np.linalg.norm(sig_A[i] - sig_B[i])
        # elif style == 'DTW':
        #     alignment = dtw(sig_A[i], sig_B[i], distance_only=True)
    ...
    if aggregate:
        ...
        if baseline is not None:
            ...
            psd_array = scale_results(psd_array, baseline['psd_mean'][-1])  # 因为只需要获得所有频段的结果，所以做了简化处理
        ...
        # phase_mean = np.mean(phase_array)
        # phase_std = np.std(phase_array, ddof=1)
        ...
        # shrunk_mag_dist = shrinkage_estimator(weights, mag_mean)
        # phase_array = np.asarray(phase_dist)
        # phase_mean = np.mean(phase_array, axis=1)
        # phase_std = np.std(phase_array, axis=1, ddof=1)
        ...
        for p, d in patient_result.items():
            ...
            if baseline is not None:
                ...
                # print('baseline psd:', baseline['psd_mean'][-1])
                # print('psd_array before:', d['psd_dist'])
                d['psd_dist'] = scale_results(d['psd_dist'], baseline['psd_mean'][-1])  # 因为只需要获得所有频段的结果，所以做了简化处理
                # print('psd_array after:', d['psd_dist'])
            ...
            # phase_mean = np.mean(d['phase_dist'])
            # phase_std = np.std(d['phase_dist'], ddof=1)
...
def get_neighbors_distance(neighbor_pos, target_pos, nearest_k=None, method='EUD'):
    '''
    返回距离channel最近的K个邻居且包含channel，以及通过method计算出的距离
    :param neighbor_pos: dict, 邻居电极坐标
    :param target_pos: array like, (3, ), 目标SEEG的三元组坐标
    :param nearest_k: int, 最近邻居个数（包含当前的EEGchannel）
    :param method: str, 计算距离方式
    :return: 邻居的名称以及距离
    '''
...
def EUD_interpolate(segment, dist, save_dir=None, save_name=None):
    ...
    :param segment: 含有多个channel的片段
    :param dir: 片段所在目录
    :param neighbors: 邻居电极名字
    :param dist: 距离
    :return:
    '''
...
def spline_interpolate(raw, pos_dict, seeg_name):
    '''
    使用mne自带的spline插值生成seeg
    :param raw: mne.io.Raw, 预处理好的raw实例
    :param pos_dict: 用来插值的eeg以及seeg电极的名称-三维坐标字典
    :param seeg_name: 目标seeg名称
    :return: mne.io.Raw，返回插值得到的结果，记得自己切割再保存
    '''
    ...
    # raw.filter(1., 28, fir_design='firwin')
    # eeg_pos = dict()
    # chs = ['EEG F1-Ref', 'EEG FZ-Ref', 'EEG F3-Ref-1', 'EEG F4-Ref-1', 'EEG CZ-Ref', 'EEG Fp2-Ref']
    # for k, v in mat_file.eeg_coor.items():
    #     for ch in eeg_ch:
...
def explore_eeg_seeg_dist_sim_relation(patient, distFile, bdSimFile, hdSimFile, saveDir=None):
    '''
    画出每个病人每根EEG电极与所有SEEG电极之间的距离vs相似度的关系（散点图+直线拟合）
    :param patient: str, 病人名称
    :param distFile: str, 坐标距离文件路径
    :param bdSimFile: str, bd相似度文件路径
    :param hdSimFile: str, hd相似度文件路径
    :param saveDir: str, 存储目录
    :return: (float, float), 两种相似度下拟合直线斜率为负的比例
    '''
    ...
    # eegChans = random.sample(distDict.keys(), n_sample)
    ...
    for i, chan in enumerate(eegChans):
        ...
        for intv_idx in range(len(distIntv) - 1):
            span = np.where((fullX > distIntv[intv_idx]) & (fullX <= distIntv[intv_idx + 1]), True,
                            False)  # 筛选出符合区间的值，返回对应位置是True是False
            if np.any(span):
                X = fullX[span]
                Y1 = fullY1[span]
    ...
    # return bdNegSlope * 1. / n_sample, hdNegSlope * 1. / n_sample

def explore_patient_dist_sim_relation(patient, distFile, bdSimFile, hdSimFile, saveDir=None):
    '''
    画出每个病人所有EEG电极与所有SEEG电极之间的距离vs相似度的关系（散点图+直线拟合）
    :param patient: str, 病人名称
    :param distFile: str, 坐标距离文件路径
    :param bdSimFile: str, bd相似度文件路径
    :param hdSimFile: str, hd相似度文件路径
...
def explore_segment_dist_sim_relation(patient, distFile, saveDir=None):
    '''
    通过均匀抽样28秒长度的时间片，画出每个病人每个EEG电极与所有SEEG电极之间的距离vs相似度的关系（散点图+直线拟合）
    :param patient: str, 病人名称
    :param distFile: str, 坐标距离文件路径
    :param saveDir: str, 存储目录
    :return:
    ...
    rawEEG = read_raw_signal(os.path.join("/home/hmq/Signal/preprocessed/", patient + '_eeg_raw.fif'))
...
def explore_segment_range_dist_sim_relation(patient, distFile, intvLen, saveDir=None):
    '''
    通过均匀抽样28秒长度的时间片，根据距离区间，画出每个病人每个EEG电极与所有SEEG电极之间的距离vs相似度的关系（散点图+直线拟合）
    :param patient: str, 病人名称
    :param distFile: str, 坐标距离文件路径
    :param intvLen: int, 距离间隔
    :param saveDir: str, 存储目录
    :return:
    '''
    ...
    return {'mean bdNegRate across exper': np.mean(bdNegRate, axis=2), 'mean hdNegRate across epxer': np.mean(hdNegRate, axis=2),
            ...
            # 'mean bdNegRate': np.nanmean(np.where(bdNegRate != 0, bdNegRate, np.nan), axis=1),
            # 'mean hdNegRate': np.nanmean(np.where(hdNegRate != 0, hdNegRate, np.nan), axis=1),
...
def calculate_rmse(real_path, fake_path, normalizer, method='GAN', is_IF=False, aggregate=True, save_path=None, baseline=None):
    ...
    for i, f_name in enumerate(dataset):
        ...
        for j, tup in enumerate(bands):
            ...
            if j == len(bands) - 1:
                ...
                # patient_result[patient]['mag_cov'].append(np.sum((r_mag - r_mag.mean()) * (f_mag - f_mag.mean())) / (r_mag.size - 1))
                ...
                patient_result[patient]['mag_mssmi'].append(compare_ssim(r_mag, f_mag, gaussian_weights=True))
                patient_result[patient]['phase_mssmi'].append(compare_ssim(r_p, f_p, gaussian_weights=True))
                patient_result[patient]['IF_mssmi'].append(compare_ssim(r_IF, f_IF, gaussian_weights=True))
    ...
    if aggregate:
        ...
        if baseline is not None:
            # mag_mssmi_array = scale_results(mag_mssmi_array, baseline['mag_mssmi'])
...
def compare_by_interval(real_path, fake_path, normalizer, n_interval, method='GAN', is_IF=False, aggregate=True, save_path=None):
    ...
    for i, f_name in enumerate(dataset):
        ...
        for j in range(n_interval):
            ...
            # dtw_dist = dtw.distance_fast(r_temp[start: end], fake[start: end], use_pruning=True)
            rmse_dist = np.linalg.norm(r_temp[start: end] - fake[start: end], ord=2) / (end - start) ** 0.5
            # patient_result[patient]['dtw_interval'][j].append(dtw_dist)
            patient_result[patient]['rmse_interval'][j].append(rmse_dist)

    result = None
    if aggregate:

        # dtw_interval_array = [[] for _ in range(n_interval)]
        rmse_interval_array = [[] for _ in range(n_interval)]

        for _, d in patient_result.items():
            for i in range(n_interval):
                # dtw_interval_array[i].extend(d['dtw_interval'][i])
                rmse_interval_array[i].extend(d['rmse_interval'][i])

        # dtw_interval_mean = np.mean(dtw_interval_array, axis=1)
        # dtw_interval_std = np.std(dtw_interval_array, axis=1, ddof=1)
        rmse_interval_mean = np.mean(rmse_interval_array, axis=1)
        rmse_interval_std = np.std(rmse_interval_array, axis=1, ddof=1)

        result = {#'dtw_interval_mean': dtw_interval_mean, 'dtw_interval_std': dtw_interval_std,
                  'rmse_interval_mean': rmse_interval_mean, 'rmse_interval_std': rmse_interval_std
                  }
    else:
        result = {}
        for p, d in patient_result.items():

            # dtw_interval_mean = np.mean(d['dtw_interval'], axis=1)
            # dtw_interval_std = np.std(d['dtw_interval'], axis=1, ddof=1)
            rmse_interval_mean = np.mean(d['rmse_interval'], axis=1)
            rmse_interval_std = np.std(d['rmse_interval'], axis=1, ddof=1)

            result[p] = {#'dtw_interval_mean': dtw_interval_mean, 'dtw_interval_std': dtw_interval_std,
                  'rmse_interval_mean': rmse_interval_mean, 'rmse_interval_std': rmse_interval_std
                  }
