...
class PIX2PIXGANSYNTHModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        netEEGEncoder = networks.EEGEncoder(opt.input_nc, opt.ngf)
        netG = PGGAN.Generator(16, opt.ngf * 8, opt.output_nc,
        ...
        if len(self.gpu_ids) > 0:
            ...
            self.netEEGEncoder = networks.init_net(opt, netEEGEncoder, init_type=opt.init_type, init_gain=opt.init_gain, gpu_ids=self.gpu_ids, device=self.device)
        ...
        if self.isTrain:
            ...
            netD = PGGAN.Discriminator(16, opt.input_nc + opt.output_nc, is_sigmoid=self.is_sigmoid_,
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG.parameters(), self.netEEGEncoder.parameters()), lr=opt.g_lr, betas=(opt.beta1, 0.999))
    ...
    def set_input(self, input):
        ...
        real_A_label = self.normalizer.normalize(input['A' if self.AtoB else 'B'], 'seeg' if self.AtoB else 'eeg')
        ...
        # while width_A != self.bottleneck_width:  # eeg
        ...
        while width_B != current_level_res:  # 压缩真实图像 seeg
            ...
            real_A_label = self.avg_layer(real_A_label)
            width_B /= 2

        self.real_A = real_A_image.to(self.device)
        self.real_A_label = real_A_label.to(self.device)
    ...
    def forward(self):  # 先set_net_level再set_input再config_net再forward
        ...
        latent_vector = self.netEEGEncoder(self.real_A)
        self.fake_B = self.netG(latent_vector)  # 这里的输入改成脑电/噪音
