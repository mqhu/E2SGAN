...
from modules import MutualAttention
...
class Pix2PixAEModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_real', 'D_fake', 'Wasserstein', 'D', 'D_grad', 'D_eeg', 'G', 'G_L1', 'G_GAN', 'mag_L1', 'phase_L1']
        ...
        if self.isTrain:
            ...
            self.amplified_matrix = amplified_matrix.index_fill(2, torch.LongTensor(range(40, 128)), 0.2).to(self.device)
    ...
    def backward_G(self):
        ...
        # fake_phase_diff = self.fake_B[:, 1:, :, :] - self.real_A[:, 1:, :, :]
        # fake_AB = torch.cat((self.real_A[:, :1, :, :], self.fake_B[:, :1, :, :]), 1)  # 这里不用把fake_attn_feat给detach，因为backward前已经把optimizer_D里面的grad都设为false了
        fake_AB = torch.cat((self.real_A, self.fake_B), 1)
        pred_fake = self.netD(fake_AB)
