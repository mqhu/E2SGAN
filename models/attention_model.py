...
class AttentionModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--ae_name', type=str, default="", help='name of AE model')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_real', 'D_fake', 'Wasserstein', 'D', 'D_grad']
        ...
        AE_net = AEModel(opt, to_gpu=False)
        if self.isTrain:
            if not opt.train_AE:
                AE_net.load_networks('latest', path=os.path.join('../experiments', opt.checkpoints_dir, opt.ae_name))
        if self.AtoB:
            self.netEn_forward = AE_net.netAE_s.encoder  # seeg Encoder
            self.netDe = AE_net.netAE_e.decoder  # eeg Decoder
        else:
            self.netEn_forward = AE_net.netAE_e.encoder  # eeg Encoder
            self.netDe = AE_net.netAE_s.decoder  # seeg Decoder
        netG = networks.VGG_generator(self.netEn_forward, self.netDe)
        # VGG_ae = networks.VGG16_AE(input_nc=opt.input_nc, output_nc=opt.output_nc)
        # if self.isTrain:
        #     opt.train_AE = True
        #     if opt.train_AE:
        #         VGG_ae.init_network()
        # self.netEn_forward = VGG_ae.encoder  # seeg Encoder
        # # netEn_backward = AE_net.netAE_e.module.encoder  # eeg Encoder
        # self.netDe = VGG_ae.decoder  # eeg Decoder
        # netG = networks.VGG_generator(self.netEn_forward, self.netDe)
        netAttention = SelfAttention(conf.w, conf.h)
        # netE_s = networks.AE_component(opt, 'seeg', 'encoder', 1, 1)  # SEEG encoder
        if len(self.gpu_ids) > 0:
            assert (torch.cuda.is_available())
            netAttention.to(self.gpu_ids[0])
            netG.to(self.gpu_ids[0])
            # netEn_backward.to(self.gpu_ids[0])
            #netE_s.to(self.gpu_ids[0])
            self.netAttention = torch.nn.DataParallel(netAttention, self.gpu_ids)
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            self.netD = networks.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD, 2,  # 为了测试D的输入修改了维度
                                           opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = AdaBelief(self.netG.parameters(), lr=opt.g_lr, eps=1e-16, betas=(0.9, 0.999),  #这个参数影响很大！！
    ...
    def set_input(self, input):
        ...
        real_A = self.normalizer.normalize(input['A' if self.AtoB else 'B'], 'seeg' if self.AtoB else 'eeg').to(self.device)
        self.real_A = torch.sum(real_A, dim=2) / 3.0  # 把spec,phase,pos维度相加
    ...
    def forward(self):
        ...
        # self.avg_real_A = self.netAttention(self.real_A, self.real_A, self.real_A)  # 这里需要比较padding和求平均两种
        self.avg_real_A = self.real_A
        # print('avg_real_A:', self.avg_real_A)
        self.fake_B = self.netG(self.avg_real_A)  # G(A)
        # print('fake B:', self.fake_B)
    ...
    def backward_D(self):
        ...
        # one_ch_real_A = torch.sum(self.avg_real_A, dim=1, keepdim=True) / (1.0 * self.avg_real_A.shape[1])
        fake_AB = torch.cat((self.avg_real_A, self.fake_B), 1)
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        #real_AB = torch.cat((self.real_A, self.real_B), 1)
        real_AB = torch.cat((self.avg_real_A, self.real_B), 1)
