...
import sys
import os
#from models import myvgg16
from SwinTransformer.models.swin_transformer import SwinTransformer
import numpy as np
import random
from gansynth.PGGAN import PixelWiseNormLayer, EqualizedLearningRateLayer
from modules import MutualAttention, OneSidedAttention
from gansynth.PGGAN import EqualizedLearningRateLayer, PixelWiseNormLayer
...
def init_net(opt, net, init_type='normal', init_gain=0.02, gpu_ids=[], device=None):
    ...
    if len(gpu_ids) > 0:
        ...
        if opt.isTrain and 'eeggan' not in opt.model and len(gpu_ids) > 0:
            ...
            net = torch.nn.DataParallel(net, [0])  # 如果多个gpu id会和train时的distributeddataparallel冲突
            # net = torch.nn.DataParallel(net, gpu_ids)
...
def define_D_vgg(input_nc, gpu_ids=[], pretrained=True, pre_path=None, norm='batch', use_dropout=False):
    """Create a discriminator
    ...
    The discriminator has been initialized by <init_net>. It uses Leakly RELU for non-linearity.
    """
    net = VGG16(input_nc, pretrained=pretrained, pre_path=pre_path, norm=norm, use_dropout=use_dropout)
...
def cafe_ver_vgg16(path):
    '''
    据说pytorch自带的vgg实现没有训练充分，此函数加载caffe版本vgg16转化为pytorch版本，参考https://github.com/jcjohnson/pytorch-vgg
    :param path: vgg16的pth路径
    :return:vgg16 模型
    '''
    net = models.vgg16()
    pre = torch.load(path)
    # pre = OrderedDict([(map[k] if k in map else k, v) for k, v in pre.iteritems()])
...
class GANLoss(nn.Module):
    ...
    def __call__(self, prediction, target_is_real):
        ...
        if self.gan_mode in ['lsgan', 'vanilla']:
            ...
            if target_is_real:
                loss = -prediction.mean()  # 返回的是tensor
            else:
...
def cal_gradient_penalty(netD, real_data, fake_data, device, type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    if lambda_gp > 0.0:
        if type == 'real':   # either use real images, fake images, or a linear interpolation of two.
            ...
            #print("alpha1:" + str(alpha.shape))
            alpha = alpha.expand(real_data.shape[0], real_data.nelement() // real_data.shape[0]).contiguous().view(*real_data.shape)
            #print("alpha2:" + str(alpha.shape))
            #print("real:" + str(real_data.shape))
            #print("fake:" + str(fake_data.shape))
    ...
    def __init__(self, input_nc, output_nc, ngf=64, convD=2, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        if convD == 3:
            oneD_opr1 = [nn.Conv3d(seeg_chans, eeg_chans, kernel_size=1, stride=1, padding=0, bias=use_bias),
                    ...
                    nn.ReLU(True)]  # 用1x1kernel改变channel个数适应不同尺寸输入输出
        ...
        if convD == 3:
            oneD_opr2 = [nn.Conv3d(24, eeg_chans, kernel_size=1, stride=1, padding=0, bias=use_bias),  # 24是上采样的结果
                         norm_layer(eeg_chans),
                         nn.ReLU(True)]
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias, convD):
        ...
        conv_block += [EqualizedLearningRateLayer(conv_block[-1]), nn.LeakyReLU(negative_slope=0.2), PixelWiseNormLayer()]
    ...
    def forward(self, x):
        ...
        out = self.relu(out)  # pix2pix的实现里没有这一步，但标准的ResNet有
        return out
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        upnorm = FrequencyNorm(outer_nc, feat_w)
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, convD=2, aux=[]):
        ...
        #oneD_opr = [nn.Conv3d(2, 32, kernel_size=1, stride=1, padding=0, bias=use_bias),  #  spec,IF维升维
        #            norm_layer(32),
        #            nn.ReLU(True)]  # 用1x1kernel改变channel个数适应不同尺寸输入输出
        ...
        if 'attn' in aux:
            ...
            input_nc += 2
            # self.attn_norm = EqualizedLearningRateLayer(self.attn_block)
        ...
        if convD == 2:
            if 'linear' in aux:
                ...
                # self.lin = nn.Linear(ndf * nf_mult * 15 * 15, 1)
                # self.global_patch = nn.Sequential(nn.Linear(14 * 14, 1), nn.LeakyReLU(0.2, True))
                self.nonlin = nn.LeakyReLU(0.2, True)
                self.fuse = nn.Linear(14 * 14 + 1, 1)
                # self.voting_weights = nn.Parameter(torch.Tensor(14 * 14))
            else:
                sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw, bias=use_bias)]
        elif convD == 3:
    ...
    def forward(self, x):
        ...
        if 'attn' in self.aux or 'onesided' in self.aux:
            ...
            # x = self.model(torch.cat((x, attn_feat), 1))
        elif 'corrcoef' in self.aux:
            corrcoefmat = self.corrcorf_block(x)
            x = torch.cat((x, corrcoefmat), 1)
            # x = self.model(torch.cat((x, corrcoefmat), 1))

        x = self.model(x)

        if 'linear' in self.aux:
            # x = self.model(x)
            # print('before lin shape:', x.shape)
            x = x.reshape(x.size()[0], -1)
            # print('resized shape:', x.shape)
            # print('lin input shape:', self.in_size)
            # global_view = self.global_patch(x)
            global_view = self.nonlin(x.mean(axis=1, keepdims=True))
            x = self.fuse(torch.cat((x, global_view), 1))
            # x = x * self.voting_weights
        return x
        # if 'attn' in self.aux or 'onesided' in self.aux:
        #     return x, self.attn_block.attn, self.attn_block.query, self.attn_block.key
        # else:
        #     return x
...
class VGG16_AE(nn.Module):
    ...
    def __init__(self, version, pre_path=None, input_nc=2, output_nc=2, ngf=32, norm_layer=nn.InstanceNorm2d, use_bias=True):
        ...
        if version == 11:
            ...
            net = models.vgg16()  # 避免加入batch norm
        if pre_path is not None:
            pre = torch.load(pre_path)
            net.load_state_dict(pre)

        norm_layer = get_norm_layer('instance')
        en_features = [nn.Conv2d(input_nc, 3, kernel_size=1, stride=1, padding=0, bias=False)]  # 增加1x1卷积升维
        if version == 11:
            en_features += list(net.features)[: 10]  # 取到filter数为512为止
        else:
...
class VGG_generator(nn.Module):
    ...
    def __init__(self, vgg_version, encoder, decoder):
        ...
        #AE_net = AEModel(opt)
        #AE_net.load_networks('latest', path=os.path.join(opt.checkpoints_dir, opt.ae_name))

        norm_layer = get_norm_layer('instance')
        self.encoder = encoder  # 为了给pix2pix计算latent vector用
        ...
        # down_0 = encoder[24:]  把每个切片作为一个self属性，encoder当成一个列表，当要用的时候再转换成sequential
        # down_1 = encoder[14: 24]
        # down_2 = encoder[7: 14]
        # down_3 = encoder[: 7]
        if vgg_version == 11:
            k_size = 256
        ...
        # unet_block = UnetSkipConnectionBlock(output_nc=256, submodule=None, innermost=True, down=down_0, up=up_0)  # add the innermost layer
        # unet_block = UnetSkipConnectionBlock(output_nc=128, submodule=unet_block, down=down_1, up=up_1)
        # unet_block = UnetSkipConnectionBlock(output_nc=64, submodule=unet_block, down=down_2, up=up_2)
        # self.model = UnetSkipConnectionBlock(submodule=unet_block, down=down_3, up=up_3, outermost=True)
        # self.mid_hook = 0
        # self.out_hook = 0
...
class AE_component(nn.Module):
    ...
    def __init__(self, opt, type, component, eeg_chans=None, seeg_chans=None):
        ...
        netAE.load_networks('latest', path=os.path.join(opt.checkpoints_dir, opt.ae_name))  # 调用BaseModel自带的Load方法
...
class SwinTransformerDiscriminator(nn.Module):
    ...
    def __init__(self, preTrainPath, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.swintransformer.load_state_dict(torch.load(preTrainPath, map_location=torch.device('cpu'))['model'])  # 大坑！！不map到CPU上，DDP会负载不均衡
    ...
    def forward(self, x):
        ...
        x = self.oneDconv(x)  # 用0去padding第三维
...
class CorrcoefMatrix(nn.Module):
    ...
    def corrcoef(self, x, y):
        ...
        xvar_ = x.var(axis=-1)#.reshape(n_batch, n_feat, 1, x.shape[1])
        yvar_ = y.var(axis=-1)#.reshape(y.shape[1], 1)
