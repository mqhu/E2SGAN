...
class Pix2PixSwinModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_real', 'D_fake', 'Wasserstein', 'D', 'D_grad', 'D_eeg', 'G_GAN', 'G_L1', 'G']
        ...
        '''
        self.netG = networks.define_G(opt, opt.input_nc, opt.output_nc, opt.ngf, 'resnet', n_blocks=3, norm=opt.norm,
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            netD = networks.SwinTransformerDiscriminator(preTrainPath="/home/hmq/Projects/swin_tiny_patch4_window7_224.pth").cuda()
            self.netD = torch.nn.parallel.DistributedDataParallel(netD, device_ids=[opt.local_rank],
        ...
        if self.isTrain:
            ...
            self.optimizer_D = AdaBelief(filter(lambda p: p.requires_grad, self.netD.parameters()), lr=opt.d_lr, eps=1e-12, betas=(0.5, 0.999),
                                         weight_decay=True, rectify=False, fixed_decay=False, amsgrad=False)
    ...
    def optimize_parameters(self):
        ...
        if self.train_D:
            n_alter = 5
            self.set_requires_grad([self.netD.module.swintransformer.head], True)  # enable backprop for D
            # update D
        ...
        if self.train_G and self.batch_idx % n_alter == 0:
            self.set_requires_grad([self.netD.module.swintransformer.head], False)  # D requires no gradients when optimizing G
